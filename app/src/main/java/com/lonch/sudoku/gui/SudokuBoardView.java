...
import com.lonch.sudoku.game.Cell;
import com.lonch.sudoku.game.CellCollection;
import com.lonch.sudoku.game.CellCollection.OnChangeListener;
import com.lonch.sudoku.game.CellNote;
...
public class SudokuBoardView extends View {
    ...
    private int mFontstyle, mFontstylereal;
    private int mFontsize, mFontsizereal;
	...
	public SudokuBoardView(Context context, AttributeSet attrs/*, int defStyle*/) {
		...
		mFontstyle = mFontstylereal=0;
		mFontsize = mFontsizereal =3;
		...
		setTextColorReadOnly(a.getColor(R.styleable.SudokuBoardView_textColorReadOnly, Color.BLUE));
		...
		setBackgroundColorSecondary(a.getColor(R.styleable.SudokuBoardView_backgroundColorSecondary, Color.GRAY));
		setBackgroundColorReadOnly(a.getColor(R.styleable.SudokuBoardView_backgroundColorReadOnly, Color.GRAY));
	...
	public void setdigitfontsize(int i) {
		mFontsize = i;
	}
	public void setdigitfontstyle(int i) {
		mFontstyle = i;
	}
	public void setrealfontsize(int i) {
		mFontsizereal = i;
	}
	public void setrealfontstyle(int i) {
		mFontstylereal = i;
	}
...
//        Log.d(TAG, "heightSize=" + heightSize);
		...
		float cellTextSize;
		cellTextSize = getcellTextSize(mCellHeight, mFontsize);
		mCellValuePaint.setTextSize(cellTextSize);
		int typeface = getcellFontstyle(mFontstyle);

		mCellValuePaint.setTypeface(Typeface.create(Typeface.DEFAULT, typeface));
		mCellValueInvalidPaint.setTextSize(cellTextSize);
		mCellNotePaint.setTextSize(mCellHeight / 3.0f);

		cellTextSize = getcellTextSize(mCellHeight, mFontsizereal);
		mCellValueReadonlyPaint.setTextSize(cellTextSize);
		typeface = getcellFontstyle(mFontsizereal);
		mCellValueReadonlyPaint.setTypeface(Typeface.create(Typeface.DEFAULT, typeface));
	...
	private int getcellFontstyle(int mT) {
		int t= Typeface.NORMAL;
		switch (mT) {
			case 1: t = Typeface.BOLD; break;
			case 2: t = Typeface.ITALIC; break;
			case 3: t = Typeface.BOLD_ITALIC; break;
			case 0:
			default: t = Typeface.NORMAL; break;
		}
		return t;
	}

	private float getcellTextSize(float mCellHeight, int mF) {
		float n = (float) (mCellHeight * 0.75);
		switch (mF)  {
			case 1: n = (float) (mCellHeight * 0.95); break;
			case 2: n = (float) (mCellHeight * 0.85); break;
			case 3: n = (float) (mCellHeight * 0.75); break;
			case 4: n = (float) (mCellHeight * 0.65); break;
			case 5: n = (float) (mCellHeight * 0.55); break;
			default: n = (float) (mCellHeight * 0.75); break;
		}
		return n;
	}
...
/***************
				//	if (!cell.isEditable() && hasBackgroundColorReadOnly) {
					if( ((row < 3) && (col >= 3 && col<= 5)) ||
					   ((row >= 3 && row <=5) && (col < 3)) || ((row >= 3 && row <=5) && (col>= 6)) ||
					   ((row > 5) && (col >= 3 &&  col<= 5)) )
					{
